...
function updateCartUI() {
    ...
    if (cartItems) {
        if (cart.length === 0) {
            cartItems.innerHTML = '<div class=\"cart-empty\"><i class=\"fas fa-shopping-bag\" style=\"font-size:3rem;margin-bottom:1rem;color:var(--gray-300)\"></i><p>Tu carrito está vacío</p></div>';
        } else {
...
function checkoutWhatsApp() {
    ...
    cart.forEach((item, i) => {
        ...
        total += subtotal;
        message += (i+1) + '. ' + item.name + ' (' + item.brand + ')%0A';
        message += '   Ref: ' + (item.ref || 'N/A') + ' | Tallas: ' + item.sizes + '%0A';
        message += '   Cantidad: ' + item.qty + ' x ' + formatPrice(item.price) + ' = ' + formatPrice(subtotal) + '%0A%0A';
    });
